...
def covid_deaths(apps, schema_editor):
    Covid_deaths = apps.get_model('chart', 'Covid_deaths')  # (app_label, model_name)
    csv_file = os.path.join(settings.BASE_DIR, 'covid_de.csv')
    with open(csv_file) as dataset:                   # 파일 객체 dataset
        ...
        for entry in reader:                            # 판독기에 대하여 반복 처리
            Covid_deaths.objects.create(                       # DB 행 생성
                Date=entry[Date],
...
class Migration(migrations.Migration):
    ...
    dependencies = [
        ('chart', '0007_covid_deaths'),
    ]

    operations = [
        migrations.RunPython(covid_deaths),
