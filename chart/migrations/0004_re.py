...
def covid_recovered(apps, schema_editor):
    Covid_recovered = apps.get_model('chart', 'Covid_recovered')  # (app_label, model_name)
    csv_file = os.path.join(settings.BASE_DIR, 'covid_re.csv')
    with open(csv_file) as dataset:                   # 파일 객체 dataset
        ...
        for entry in reader:                            # 판독기에 대하여 반복 처리
            Covid_recovered.objects.create(                       # DB 행 생성
                Date=entry[Date],
...
class Migration(migrations.Migration):
    ...
    dependencies = [
        ('chart', '0003_covid_recovered'),
    ]

    operations = [
        migrations.RunPython(covid_recovered),
